 ...
 * 1、Create a new uikit_layout_default_no_data.xml overlay in app Layout.
 * 2、Inheriting ChatUIKitBaseRecyclerViewAdapter, rewrite getEmptyLayoutId () method, return to the layout of the custom.
 * @param <T>
</T> */
abstract class ChatUIKitBaseRecyclerViewAdapter<T> :
    ChatUIKitBaseAdapter<ChatUIKitBaseRecyclerViewAdapter.ViewHolder<T>>() {
    ...
    private val emptyLayoutId: Int
         ...
         * @return
         */
        get() = R.layout.uikit_layout_default_no_data
    ...
    protected fun getEmptyViewHolder(parent: ViewGroup): ViewHolder<T> {
        ...
        if (isHideEmptyView) {
            emptyView = LayoutInflater.from(mContext)
                .inflate(R.layout.uikit_layout_no_data_show_nothing, parent, false)
        }
     ...
     */
    fun addData(item: T) {
        synchronized(ChatUIKitBaseRecyclerViewAdapter::class.java) {
            if (mData == null) {
                mData = ArrayList()
            }
            mData?.add(item)
    ...
    abstract class ViewHolder<T>(itemView: View ?= null, binding: ViewBinding? = null) : RecyclerView.ViewHolder(itemView ?: binding?.root!!) {
         ...
         * @return
         */
        var adapter: ChatUIKitBaseAdapter<ViewHolder<T>>? = null
            private set
         ...
         * @param adapter
         */
        fun setAdapter(adapter: ChatUIKitBaseRecyclerViewAdapter<T>) {
